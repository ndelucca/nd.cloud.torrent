...
- **A comment states the invariant that holds now, not the history of how it got there.** The "why" that stops someone reverting a decision stays; the narrative — what the code used to do, which bug was hit, which test pins it — belongs in the commit message and the test name. Both are recoverable from `git log` and `go test`, and neither goes stale in the source. This repo carried three copies of every rationale (comment, `CLAUDE.md`, test name) and they had already drifted apart in four places.
