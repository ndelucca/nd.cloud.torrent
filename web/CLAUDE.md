...
- **A tick is one broadcast, and a connect is one write.** Everything a pass produces — the skeleton, the changed rows, the forget events — is concatenated and broadcast once; SSE frames are self-delimiting, which is what makes that valid. This is what gives `subBuffer` a meaning: broadcasting per torrent made the buffer measure *changed rows* rather than lag, so eight rows moving in one tick could stall a healthy client (each frame was its own `Write` and `Flush`), and `broadcast` disconnects a subscriber it cannot deliver to — which reconnected, took the snapshot, kicked the loop, and produced another burst. `TestOneBroadcastPerTick` pins it, and fails with 8 broadcasts for 24 torrents against the old code, which is the stall happening.
- **`snapshot` takes the region that must lead, and returns one buffer.** A membership region has to arrive before the item regions whose elements it creates — an element cannot listen for `torrent-<hash>` before it exists, so earlier row frames are silently discarded. That previously held only because infohashes are lowercase hex and `'l'` sorts after `'f'`; it inverts the day hashes are encoded any other way. `TestSnapshotLeadsWithMembership` stores a row under a hash that sorts *before* the skeleton, so alphabetical order alone would fail it.
